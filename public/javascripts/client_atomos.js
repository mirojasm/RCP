...
var host=document.getElementById('clientAtom').getAttribute('host');
var port=document.getElementById('clientAtom').getAttribute('port');
...
server.on('connect', function(data) {
  ...
  $('.writing').hide();
  nivel_actual = "level1";
  server.emit('loadGame', {tipo_juego: tipo_juego(), nombre_jugador: get_sent_data()['nombre_s']});
});

server.on('test', function(data) {
  jugador2 = "Juan Protón";
  jugador3 = "Elisa Electrón";
  levelsData = JSON.parse(data.atoms_level);
  colorData = JSON.parse(data.color);
  solutionsData = JSON.parse(data.solutions_atoms);
  ...
  for (var key in solutionsData) {
    ...
    for (var line = 0; line < solutionsData[key].length; line++) {
      ...
      for (var numero = 0; numero < timesP2; numero++) {
        lista1.push(solutionsData[key][line][parseInt(maxSelectData[key][0]) + numero]);
      }
      ...
      for (var numero = 0; numero < timesP3; numero++) {
        lista2.push(solutionsData[key][line][parseInt(maxSelectData[key][0]) + parseInt(maxSelectData[key][1]) + numero]);
      }
  ...
  decision = false;
  if (cargar === "new") {
    var data = "B-"+nino_nombre+"-NUEVO JUEGO-"+"Hora-" + Date()
    server.emit('escribir_resultados', data);
  }
  startGame(set_stars(nivel_actual));
  server.emit('playGame', {tipo_juego: tipo_juego(), nombre_jugador: get_sent_data()['nombre_s']});
});
...
function reiniciarLevel() {
  ...
  P.then((successMessage) => {
    indiceSeleccion = Math.floor(Math.random() * (solutionsP2[nivel_actual].length));
...
server.on('phrase', function(data) {
  ...
  if (phrase.indexOf("XX1") != -1) {
    ...
    phrase = phrase.slice(0, inicio) + jugador2 + phrase.slice(inicio + 3, phrase.length);
  }
  if (phrase.indexOf("XX2") != -1) {
    var inicio = phrase.indexOf("XX2");
    phrase = phrase.slice(0, inicio) + jugador3 + phrase.slice(inicio + 3, phrase.length);
  }
  if (data.author.name == "P1") {
    var dat = "B-"+nino_nombre+"-mensaje-" + phrase;
    ...
    if (decision && arreglo_decision.length == 0) {
      ...
      for (var aux = 0; aux < arreglo_decision.length; aux++) {
        arreglo_decision[aux] = (parseInt(arreglo_decision[aux]) - 1).toString();
      }
    }
    ...
    if (phrase.indexOf("[#]") != -1) {
      ...
      for (var msge = 0; msge < indices.length; msge++) {
        ...
        phrase = phrase.slice(0, inicio) + lista_elegidas[msge] + phrase.slice(inicio + 3, phrase.length);
      }
      ...
      phrase = phrase.slice(0, inicio) + lista_elegidas[0] + phrase.slice(fin + 1, phrase.length);
    }
    ...
    if (phrase.indexOf("[#]") != -1) {
      ...
      if (indices.length > timesP2) {
        ...
        if(decision === true)
        {
          for (var msge = 0; msge < indices.length; msge++) {
            var inicio = phrase.indexOf("[#");
            phrase = phrase.slice(0, inicio) + (parseInt(lista_elegidas[msge]) + 1) + phrase.slice(inicio + 3, phrase.length);
          }
        }
        else{
          for (var msge = 0; msge < indices.length; msge++) {
            var inicio = phrase.indexOf("[#");
            //phrase = phrase.slice(0, inicio) + (parseInt(solutionsData[nivel_actual][indiceSeleccion][msge]) + 1).toString() + phrase.slice(inicio + 3, phrase.length);
            phrase = phrase.slice(0, inicio) + (parseInt(solutionsP2[nivel_actual][indiceSeleccion][msge]) + 1) + phrase.slice(inicio + 3, phrase.length);
          }
        }
      ...
      if (indices.length != 1) {
        ...
        for (var msge = 0; msge < indices.length; msge++) {
          var inicio = phrase.indexOf("[$");
          phrase = phrase.slice(0, inicio) + colorToSpanish(allColors[nivel_actual][msge]) + phrase.slice(inicio + 3, phrase.length);
        }
      }
      else {
        var inicio = phrase.indexOf("[$");
        phrase = phrase.slice(0, inicio) + colorToSpanish(colorP2[nivel_actual]) + phrase.slice(inicio + 3, phrase.length);
      }
    }
    ...
    if (phrase.indexOf("[") != -1) {
      ...
      phrase = phrase.slice(0, inicio) + opciones[randomChoice] + phrase.slice(fin + 1, phrase.length);
    }
    ...
    if (phrase.indexOf("[#]") != -1) {
      ...
      if (indices.length > timesP3) {
        ...
        if(decision === true)
        {
          ...
          for (var msge = 0; msge < indices.length; msge++) {
            var inicio = phrase.indexOf("[#");
            //phrase = phrase.slice(0, inicio) + (parseInt(solutionsData[nivel_actual][indiceSeleccion][msge]) + 1).toString() + phrase.slice(inicio + 3, phrase.length);
            phrase = phrase.slice(0, inicio) + (parseInt(solutionsP3[nivel_actual][indiceSeleccion][msge]) + 1) + phrase.slice(inicio + 3, phrase.length);
          }
        }
      ...
      if (indices.length != 1) {
        ...
        var inicio = phrase.indexOf("[$");
        phrase = phrase.slice(0, inicio) + colorToSpanish(colorP3[nivel_actual]) + phrase.slice(inicio + 3, phrase.length);
      }
    }
  ...
  if (data.author.name == 'Felipe') {
    $('.writing-text').text("Sofía está escribiendo ...");
    $('.writing').show();
    
    setTimeout(function() {
      $('.writing').hide();
      cl.append("<li><div class='commenterImage p1'></div><div class='commentText comment'><p class=''><strong>Sofia:</strong> " + phrase + "</p> <span class='date sub-text'>" + time + "</span></div></li>");
      cl.animate({
    ...
    $('.writing-text').text("Juan está escribiendo ...");
    $('.writing').show();
    ...
    setTimeout(function() {
      $('.writing').hide();
      cl.append("<li><div class='commenterImage p2'></div><div class='commentText comment'><p class=''><strong>" + jugador2 + ":</strong> " + phrase + "</p> <span class='date sub-text'>" + time + "</span></div></li>");
      cl.animate({
        scrollTop: cl.prop("scrollHeight")
      }, 100);
    }, 2000);
  } else if (data.author.name == 'P3') {
    $('.writing-text').text("Elisa está escribiendo ...");
    $('.writing').show();
    ...
    setTimeout(function() {
      $('.writing').hide();
      cl.append("<li><div class='commenterImage p3'></div><div class='commentText comment'><p class=''><strong>" + jugador3 + ":</strong> " + phrase + "</p> <span class='date sub-text'>" + time + "</span></div></li>");
      cl.animate({
        scrollTop: cl.prop("scrollHeight")
      }, 100);
    }, 2000);
  } else {
    cl.append("<li><div class='commenterImage p4'></div><div class='commentText comment'><p class=''><strong>" + "Tú" + ":</strong> " + phrase + "</p> <span class='date sub-text'>" + time + "</span></div></li>");
    cl.animate({
      scrollTop: cl.prop("scrollHeight")
    }, 100);
  }
...
server.on('choices', function(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    var text = data[i].Fields['Menu Text'] == "" ? data[i].Fields['Dialogue Text'] : data[i].Fields['Menu Text'];
    ...
    if (text.indexOf("XX1") != -1) {
      ...
      text = text.slice(0, inicio) + jugador2 + text.slice(inicio + 3, text.length);
    }
    if (text.indexOf("XX2") != -1) {
      var inicio = text.indexOf("XX2");
      text = text.slice(0, inicio) + jugador3 + text.slice(inicio + 3, text.length);
    }
    ...
    if (text.indexOf("[#]") != -1) {
      ...
      for (var j = 0; j < lista_numeros.length; j++) {
        selector += "<option value='" + lista_numeros[j] + "'>" + lista_numeros[j] + "</option>";
      }
      selector += "</select>"
      for (var msge = 0; msge < indices.length; msge++) {
      ...
      content += "<input type='radio' name='choice' class='opcion' data-selector='true' nodeid='" + data[i].ID + "' value='" + data[i].ID + "' >" + " " + text + "<br>"
      selector = "";
      ...
      for (var jc = 0; jc < opciones.length; jc++) {
        selector += "<option value='" + opciones[jc] + "'>" + opciones[jc] + "</option>";
      }
      selector += "</select>"
      for (var msge = 0; msge < indices.length; msge++) {
        var inicio = text.indexOf("[$]");
      ...
      for (var msge = 0; msge < indices.length; msge++) {
        ...
        for (var j = 0; j < opciones.length; j++) {
          selector += "<option value='" + opciones[j] + "'>" + opciones[j] + "</option>";
        }
        selector += "</select>"
        text = text.slice(0, inicio) + selector + text.slice(fin + 1, text.length);
        //text = text.slice(0, inicio) + selector + text.slice(inicio + 3, text.length);
      }
      ...
      content += "<input type='radio' name='choice' class='opcion' data-selector='false' nodeid='" + data[i].ID + "' value='" + data[i].ID + "' >" + " " + text + "<br>";
    }
  }
  content += "<input type='submit' name='submit' value='' class='btn btn-info aChoice' disabled></div>";
...
server.on('variables', function(data) {
  ...
  if(data["show_stars"]==0)
  {
    ...
    $('.mapa').hide();
    $('.img-mapa').attr('src', '../images/ETAPAS_A/'+imgmapa[""+etapas[0]+etapas[1]+etapas[2]+etapas[3]]);
  }
  else{
    layer.hide();
    $('.mapa').show();
  ...
  if(varLevel != data["level"])
  {
    ...
    var P = new Promise((resolve, reject) => {
      if (varLevel === 2) {
          $("#obj_game").attr("src", "../images/level_2_4.png");
          nivel_actual = "level4";
          resolve("level4");
        } else if (varLevel === 3) {
          $("#obj_game").attr("src", "../images/level_2_6.png");
          nivel_actual = "level6";
          resolve("level6");
        } else if (varLevel === 4) {
          $("#obj_game").attr("src", "../images/level_2_8.png");
          nivel_actual = "level8";
          resolve("level8");
        }
    });
...
server.on('conversationEnd', function(data) {
  var data = "B-"+nino_nombre+"-Estado-Tiempo para elegir las estrellas:"
  server.emit('escribir_resultados', data);
  $('#choicesTime').hide();
  $('#selectionTime').show();
  $('#selectionTimeButton').replaceWith("<input type='submit' name='submit' value='Enviar tus estrellas' id='selectionTimeButton' class='btn-group-vertical btn btn-info aChoice2' disabled>");
...
var sendButtonClick = function(){
    var data = "B-"+nino_nombre+"-Estado-CLICK para enviar estrellas."
    server.emit('escribir_resultados', data);
    $('.erase_button').hide();
    estrellas_finales = estrellas_seleccionadas.concat(opcionSeleccionarP2).concat(opcionSeleccionarP3);
    ...
    if (decision) {
      var data = "B-"+nino_nombre+"-Estado-Se va a comparar con la decisón tomada por el niño."
      server.emit('escribir_resultados', data);
      var data = "B-"+nino_nombre+"-Estado-El resultado correcto es: " + JSON.stringify(b);
      server.emit('escribir_resultados', data);
      for (var select = 0; select < estrellas_seleccionadas.length; select++) {
        if (arreglo_decision.indexOf(estrellas_seleccionadas[select].toString()) == -1) {
          arreglo_decision.push(estrellas_seleccionadas[select].toString());
        }
      }
      var data = "B-"+nino_nombre+"-Estado-Los jugadores han seleccionado las estrellas: " + JSON.stringify(arreglo_decision);
      server.emit('escribir_resultados', data);
      ...
      var data = "B-"+nino_nombre+"-Estado-Se va a comparar con los valores por defecto de los autómatas."
      server.emit('escribir_resultados', data);
      var data = "B-"+nino_nombre+"-Estado-El niño ha elegido las estrellas: " + JSON.stringify(estrellas_seleccionadas)
      server.emit('escribir_resultados', data);
      var data = "B-"+nino_nombre+"-Estado-Las estrellas del niño junto con los autómatas son: " + JSON.stringify(estrellas_finales)
      server.emit('escribir_resultados', data);
      var data = "B-"+nino_nombre+"-resultado-"+nivel_actual+"-El resultado correcto es: " + JSON.stringify(b);
      server.emit('escribir_resultados', data);
    ...
    if (aparece) {
      ...
      var data = "B-"+nino_nombre+"-resultado-"+nivel_actual+"RESULTADO CORRECTO";
      server.emit('escribir_resultados', data);
      var data = "B-"+nino_nombre+"-nodo-" + opcion;
      ...
      var data = "B-"+nino_nombre+"-resultado-"+nivel_actual+"RESULTADO INCORRECTO";
      server.emit('escribir_resultados', data);
      if(nivel_actual=="level1")
      {
        etapas[0] = 1;
...
function startGame(data){  
  ...
  mensajeNoAun = createText("Aun no puedes seleccionar átomos");
  mensajeColorMalo = createText("Debes seleccionar átomos de tu color");
  mensajeMaximoPermitidas = createText("Ya seleccionaste el número máximo");
...
function addStar(i){
  ...
  stars[i].on('click tap', function(){
    if (time_to_select == true) {
      if (num_seleccionadas < current_max_select && estrellas_seleccionadas.indexOf(i) == -1) {
        if (current_color == starsArray[i].realColor || current_second_color == starsArray[i].realColor || nivel_actual == "level6" || nivel_actual == "level8") {
          var data = "B-"+nino_nombre+"-Estado-CLICK en átomos. RESULT: átomo correctamente seleccionada."
          server.emit('escribir_resultados', data);
          var data = "B-"+nino_nombre+"-Estado-El usuario ha seleccionado el átomo " + i;
          server.emit('escribir_resultados', data);
          selectStar(i);
          num_seleccionadas += 1
          estrellas_seleccionadas.push(i)
          var data = "B-"+nino_nombre+"-Estado-Átomos seleccionadas hasta el momento: " + JSON.stringify(estrellas_seleccionadas);
          server.emit('escribir_resultados', data);

          $('.erase_button').show();
          $("#chat_button").prop('disabled', false);
        } else if (mensajePantalla == false){
          var data = "B-"+nino_nombre+"-Estado-CLICK en átomos. RESULT: color incorrecto -> átomo no seleccionada.";
          server.emit('escribir_resultados', data);
    ...
    } else if (num_seleccionadas >= current_max_select && estrellas_seleccionadas.indexOf(i) == -1 && mensajePantalla == false) {
        var data = "B-"+nino_nombre+"-Estado-CLICK en átomos. RESULT: número máximo de átomos elegidas -> átomo no seleccionada.";
        server.emit('escribir_resultados', data);
        if(mensajePantalla == false)
        {
          mensajePantalla = true;
        ...
        var data = "B-"+nino_nombre+"-Estado-CLICK en átomo. RESULT: no es momento de elegir átomos -> átomo no seleccionada.";
        server.emit('escribir_resultados', data);
        mensajePantalla = true;
        mensajeNoAun.show();
        layer.draw();
      ...
      if (parseInt(num_seleccionadas) == parseInt(current_max_select)) {
        var data = "B-"+nino_nombre+"-Estado-numero maximo de átomos coincide con las seleccionadas. Se activa botón para enviar respuestas.";
        server.emit('escribir_resultados', data);
  ...
  numbers[i].on('click tap', function(){
    if (time_to_select == true) {
      if (num_seleccionadas < current_max_select && estrellas_seleccionadas.indexOf(i) == -1) {
        if (current_color == starsArray[i].realColor || current_second_color == starsArray[i].realColor || nivel_actual == "level6" || nivel_actual == "level8") {
          var data = "B-"+nino_nombre+"-Estado-CLICK en átomos. RESULT: átomos correctamente seleccionada."
          server.emit('escribir_resultados', data);
          var data = "B-"+nino_nombre+"-Estado-El usuario ha seleccionado el átomo " + i;
          server.emit('escribir_resultados', data);
          selectStar(i);
        ...
        var data = "B-"+nino_nombre+"-Estado-CLICK en átomo. RESULT: no es momento de elegir átomo -> átomo no seleccionada.";
        server.emit('escribir_resultados', data);
        mensajePantalla = true;
        mensajeNoAun.show();
        layer.draw();
    ...
    if (parseInt(num_seleccionadas) == parseInt(current_max_select)) {
        var data = "B-"+nino_nombre+"-Estado-numero maximo de átomo coincide con las seleccionadas. Se activa botón para enviar respuestas.";
        server.emit('escribir_resultados', data);
        $('#selectionTimeButton').prop('disabled', false);
      }
  });
...
// quita la seleccion de una estrella
  var data = "B-"+nino_nombre+"-Estado-CLICK en BOTÓN para deshacer selección. Número de átomos elegidas = 0";
  server.emit('escribir_resultados', data);
  layer2.clear();
  layer2 = new Kinetic.Layer();
  stage.add(layer2);
...
function set_stars(level){
  var data = "B-"+nino_nombre+"-" + level.toUpperCase();
  server.emit('escribir_resultados', data);
  ...
  for (var i = 0; i < stars_current_level.length; i++) {
    ...
    starsArray[0]  = {realColor: stars_current_level[0], posX: 0.1, posY: 0.05};
    starsArray[1]  = {realColor: stars_current_level[1], posX: 0.2, posY: 0.05};
    starsArray[2]  = {realColor: stars_current_level[2], posX: 0.3, posY: 0.05};
    starsArray[3]  = {realColor: stars_current_level[3], posX: 0.4, posY: 0.05};
    starsArray[4]  = {realColor: stars_current_level[4], posX: 0.5, posY: 0.05};
    starsArray[5]  = {realColor: stars_current_level[5], posX: 0.6, posY: 0.05};
    starsArray[6]  = {realColor: stars_current_level[6], posX: 0.7, posY: 0.05};

    starsArray[7]  = {realColor: stars_current_level[7], posX: 0.1, posY: 0.1};
    starsArray[8]  = {realColor: stars_current_level[8], posX: 0.2, posY: 0.1};
    starsArray[9]  = {realColor: stars_current_level[9], posX: 0.3, posY: 0.1};
    starsArray[10] = {realColor: stars_current_level[10], posX: 0.4, posY: 0.1};
    starsArray[11] = {realColor: stars_current_level[11], posX: 0.5, posY: 0.1};
    starsArray[12] = {realColor: stars_current_level[12], posX: 0.6, posY: 0.1};
    starsArray[13] = {realColor: stars_current_level[13], posX: 0.7, posY: 0.1};

    starsArray[14] = {realColor: stars_current_level[14], posX: 0.1, posY: 0.15};
    starsArray[15] = {realColor: stars_current_level[15], posX: 0.2, posY: 0.15};
    starsArray[16] = {realColor: stars_current_level[16], posX: 0.3, posY: 0.15};
    starsArray[17] = {realColor: stars_current_level[17], posX: 0.4, posY: 0.15};
    starsArray[18] = {realColor: stars_current_level[18], posX: 0.5, posY: 0.15};
    starsArray[19] = {realColor: stars_current_level[19], posX: 0.6, posY: 0.15};
    starsArray[20] = {realColor: stars_current_level[20], posX: 0.7, posY: 0.15};

    starsArray[21] = {realColor: stars_current_level[21], posX: 0.1, posY: 0.2};
    starsArray[22] = {realColor: stars_current_level[22], posX: 0.2, posY: 0.2};
    starsArray[23] = {realColor: stars_current_level[23], posX: 0.3, posY: 0.2};
    starsArray[24] = {realColor: stars_current_level[24], posX: 0.4, posY: 0.2};
    starsArray[25] = {realColor: stars_current_level[25], posX: 0.5, posY: 0.2};
    starsArray[26] = {realColor: stars_current_level[26], posX: 0.6, posY: 0.2};
    starsArray[27] = {realColor: stars_current_level[27], posX: 0.7, posY: 0.2};

    starsArray[28] = {realColor: stars_current_level[28], posX: 0.1, posY: 0.25};
    starsArray[29] = {realColor: stars_current_level[29], posX: 0.2, posY: 0.25};
    starsArray[30] = {realColor: stars_current_level[30], posX: 0.3, posY: 0.25};
    starsArray[31] = {realColor: stars_current_level[31], posX: 0.4, posY: 0.25};
    starsArray[32] = {realColor: stars_current_level[32], posX: 0.5, posY: 0.25};
    starsArray[33] = {realColor: stars_current_level[33], posX: 0.6, posY: 0.25};
    starsArray[34] = {realColor: stars_current_level[34], posX: 0.7, posY: 0.25};

    starsArray[35] = {realColor: stars_current_level[35], posX: 0.1, posY: 0.3};
    starsArray[36] = {realColor: stars_current_level[36], posX: 0.2, posY: 0.3};
    starsArray[37] = {realColor: stars_current_level[37], posX: 0.3, posY: 0.3};
    starsArray[38] = {realColor: stars_current_level[38], posX: 0.4, posY: 0.3};
    starsArray[39] = {realColor: stars_current_level[39], posX: 0.5, posY: 0.3};
    starsArray[40] = {realColor: stars_current_level[40], posX: 0.6, posY: 0.3};
    starsArray[41] = {realColor: stars_current_level[41], posX: 0.7, posY: 0.3};
